function wall(parent, children, style, min) {
  ...
  if (!window.getComputedStyle(b).height) {
    return;
  }
  ...
  if (rows.length >= 2 && rows[rows.length - 1].length < min) {
    ...
    if (rows[rows.length - 2].length - needed >= min) {
      for (i = 0; i < needed; i++) {
        rows[rows.length - 1] = [rows[rows.length - 2].pop()].concat(
          rows[rows.length - 1]
        );
      }
    } else {
      rows[rows.length - 2] = rows[rows.length - 2].concat(
        rows[rows.length - 1]
      );
  ...
  for (i in rows) {
    for (j in rows[i]) {
      rows[i][j].style.width =
        (getW(rows[i][j]) / getH(rows[i][j])) * 100 + "px";
      rows[i][j].style.height = 100 + "px";
    }
    var w = rows[i].reduce(function (x, y) {
      return x + getW(y);
    }, 0);
    for (j in rows[i]) {
      rows[i][j].style.height =
        (getH(rows[i][j]) * (getW(b) - 4 * rows[i].length)) / w + "px";
      rows[i][j].style.width =
        (getW(rows[i][j]) * (getW(b) - 4 * rows[i].length)) / w + "px";
...
$(function () {
  $("[data-wall-child]").each(function (i, el) {
    wall(
      "#" + el.id,
      el.attributes["data-wall-child"].value,
      el.attributes["data-wall-size"].value,
      el.attributes["data-wall-min"] ? el.attributes["data-wall-min"] : false
    );
    var sel =
      "#" + el.id + " " + el.attributes["data-wall-child"].value + " img";
