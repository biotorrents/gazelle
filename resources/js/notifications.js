function clearItem(torrentId) {
  ajax.get(
    "?action=notify_clear_item&torrentid=" + torrentId + "&auth=" + authkey,
    function () {
...
function clearSelected(filterId) {
  ...
  ajax.get(
    "?action=notify_clear_items&torrentids=" +
      checkedBoxes.join(",") +
      "&auth=" +
      authkey,
    function () {
...
$(document).ready(function () {
  ...
  notifyBoxes.keydown(function (e) {
    var nextBox,
      index = notifyBoxes.index($(this));
    if (index > 0 && e.which === 75) {
      // K
      nextBox = notifyBoxes.get(index - 1);
    } else if (index < notifyBoxes.length - 1 && e.which === 74) {
      // J
      nextBox = notifyBoxes.get(index + 1);
    } else if (e.which === 88) {
