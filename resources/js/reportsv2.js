...
function ManualResolve(reportid) {
  $("#resolve_type" + reportid)
    .append('<option value="manual">Manual Resolve</option>')
    .val("manual");
...
function Dismiss(reportid) {
  $("#resolve_type" + reportid)
    .append('<option value="dismiss">Invalid Report</option>')
    .val("dismiss");
  TakeResolve(reportid);
}
...
function ReportSelectorInit() {
  if (document.querySelector(".change_report_type")) {
    document
      .querySelector(".change_report_type")
      .addEventListener("change", ChangeReportType);
    ChangeReportType();
  }
}
