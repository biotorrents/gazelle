$(document).ready(function () {
  ...
  if (!$("input#styleurl").val()) {
    var radiobutton = $(
      'input[name="stylesheet_gallery"][value="' +
        $("select#stylesheet").val() +
        '"]'
    );
    radiobutton.click();
  ...
  $("select#stylesheet").change(function () {
    var radiobutton = $(
      'input[name="stylesheet_gallery"][value="' + $(this).val() + '"]'
    );
    radiobutton.prop("checked", true);
    $(".preview_wrapper").removeClass("selected");
    radiobutton.parent().parent().addClass("selected");
  ...
  $("#toggle_css_gallery").click(function (e) {
    ...
    $("#css_gallery").slideToggle(function () {
      $("#toggle_css_gallery").text(
        $(this).is(":visible") ? "Hide gallery" : "Show gallery"
      );
    });
  });

  document
    .querySelectorAll("[name=style_additions\\[\\]]")
    .forEach(function (el) {
      el.addEventListener("change", function (e) {
        document.body.classList.toggle("style_" + e.target.value);
  ...
  $("select#stylesheet").on("change", function (e) {
    ...
    userstyle.href = userstyle.href.replace(/\w+\.css/i, changetext + ".css");
