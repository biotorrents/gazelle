...
class Subscriptions
{
    ...
    public static function has_new_subscriptions()
    {
        ...
        $NewSubscriptions = G::$cache->get_value('subscriptions_user_new_' . G::$user['ID']);
        if ($NewSubscriptions === false) {
          ...
          WHERE s.UserID = " . G::$user['ID'] . "
            ...
            G::$cache->cache_value('subscriptions_user_new_' . G::$user['ID'], $NewSubscriptions, 0);
        }
        G::$db->set_query_id($QueryID);
    ...
    {
        $QuoteNotificationsCount = G::$cache->get_value('notify_quoted_' . G::$user['ID']);
        ...
        WHERE q.UserID = " . G::$user['ID'] . "
          ...
          AND (q.Page != 'collages' OR c.Deleted = '0')";
            ...
            G::$db->set_query_id($QueryID);
            G::$cache->cache_value('notify_quoted_' . G::$user['ID'], $QuoteNotificationsCount, 0);
        }
