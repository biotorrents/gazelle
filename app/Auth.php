...
class Auth # extends Delight\Auth\Auth
{
     ...
     */
    public function register(string $email, string $passphrase, string $username, string $invite = "")
    {
        $app = App::go();
        ...
        try {
            ...
            $response = $this->auth->registerWithUniqueUsername($email, $pasphrase, $username, function ($selector, $token) use ($email) {
                $app = App::go();

                # build the verification uri
                $uri = urlencode("https://{$app->env->SITE_DOMAIN}/verify/{$selector}/{$token}");
     ...
     */
    private function remember(bool $enabled)
    {
        $app = App::go();
