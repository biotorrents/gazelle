...
<div>
    ...
    <div class="pad">
      ...
      <ul class="stats nobullet">
        <li>
          ...
          <?=Format::get_size($TotalUpload)?>
        </li>

        <li>
          <strong>Total download:</strong>
          <?=Format::get_size($TotalDownload)?>
        </li>

        <li>
          <strong>Total buffer:</strong>
          <?=Format::get_size($TotalUpload - $TotalDownload)?>
        </li>
        <br />

        <li>
          <strong>Mean ratio:</strong>
          <?=Format::get_ratio_html($TotalUpload, $TotalDownload)?>
        </li>

        <li>
          <strong>Mean upload:</strong>
          <?=Format::get_size($TotalUpload / $NumUsers)?>
        </li>

        <li>
          <strong>Mean download:</strong>
          <?=Format::get_size($TotalDownload / $NumUsers)?>
        </li>

        <li>
          <strong>Mean buffer:</strong>
          <?=Format::get_size(($TotalUpload - $TotalDownload) / $NumUsers)?>
        </li>
        <br />

        <li>
          <strong>Total request bounty:</strong>
          <?=Format::get_size($TotalBounty)?>
        </li>

        <li>
          <strong>Available request bounty:</strong>
          <?=Format::get_size($AvailableBounty)?>
        </li>
      </ul>
    </div>
  </div>
  ...
  <div class="box">
    ...
    <div class="pad">
      ...
      <ul class="stats nobullet">
        <li>
          <strong>Total seeders:</strong>
          <?=number_format($TotalSeeders)?>
        </li>

        <li>
          <strong>Total leechers:</strong>
          <?=number_format($TotalLeechers)?>
        </li>

        <li>
          <strong>Total peers:</strong>
          <?=number_format($TotalSeeders + $TotalLeechers)?>
        </li>

        <li>
          <strong>Total snatches:</strong>
          <?=number_format($TotalOverallSnatches)?>
        </li>
        ...
        <li>
          <strong>Mean seeders per torrent:</strong>
          <?=number_format($TotalSeeders / $TotalTorrents, 2)?>
        </li>

        <li>
          <strong>Mean leechers per torrent:</strong>
          <?=number_format($TotalLeechers / $TotalTorrents, 2)?>
        </li>

        <li>
          <strong>Mean snatches per torrent:</strong>
          <?=number_format($TotalSnatches / $TotalTorrents, 2)?>
        </li>
        <br />
