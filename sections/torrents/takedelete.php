...
if ($app->user->core['id'] != $UploaderID && $app->user->cant(["torrents" => "delete"])) {
    error(403);
}

if (time_ago($Time) > 3600 * 24 * 7 && $app->user->cant(["torrents" => "delete"])) {
    error('Torrent cannot be deleted because it is over one week old. If you think there is a problem, contact staff.');
}

if ($Snatches > 4 && $app->user->cant(["torrents" => "delete"])) {
...
if (isset($_SESSION['logged_user']['multi_delete'])) {
    if ($_SESSION['logged_user']['multi_delete'] >= 3 && $app->user->cant(["torrents" => "delete"])) {
...
\Gazelle\Torrents::delete_torrent($TorrentID, $GroupID);
...
\Gazelle\Torrents::write_group_log($GroupID, $TorrentID, $app->user->core['id'], 'deleted torrent ('.\Gazelle\Text::float($Size / (1024 * 1024), 2).' MB, '.strtoupper($InfoHash[1]).') for reason: '.$_POST['reason'].' '.$_POST['extra'], 0);
