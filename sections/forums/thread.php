...
<div class="thin">
  <h2>
    ...
    <a
      href="forums.php?action=viewforum&amp;forumid=<?=$ThreadInfo['ForumID']?>"><?=$ForumName?></a> ›
    <?=$ThreadTitle?>
  </h2>
  <div class="linkbox">
    <div class="center">
...
if ($ThreadInfo['NoPoll'] == 0) {
  ...
  <div class="box thin clear">
    <div class="head colhead_dark"><strong>Poll<?php if ($Closed) {
      ...
      <a href="#" onclick="$('#threadpoll').gtoggle(); log_hit(); return false;" class="brackets">View</a></div>
    ...
    if (!$RevealVoters) {
        ...
        <li<?=((!empty($UserResponse)&&($UserResponse == $i))?' class="poll_your_answer"':'')?>><?=display_str($Answer)?> (<?=number_format($Percent * 100, 2)?>%)</li>
          <li class="graph">
      ...
      foreach ($Answers as $i => $Answer) {
        ...
        <li>
          <a
            href="forums.php?action=change_vote&amp;threadid=<?=$ThreadID?>&amp;auth=<?=$LoggedUser['AuthKey']?>&amp;vote=<?=(int)$i?>"><?=display_str($Answer == '' ? 'Blank' : $Answer)?></a>
...
if (check_perms('site_moderate_forums')) {
  ...
  foreach ($Notes as $Note) {
      ...
      <tr>
        <td class="center">
          <div class="field_div textarea_wrap"><textarea id="topic_notes" name="body" cols="90" rows="3"
              onkeyup="resize('threadnotes');" style=" margin: 0px; width: 735px;"></textarea></div>
      ...
      <tr>
        ...
        <td>
          <input type="text" id="thread_title_textbox" name="title" style="width: 75%;"
            value="<?=display_str($ThreadInfo['Title'])?>"
            tabindex="2" />
        </td>
      </tr>
      <tr>
