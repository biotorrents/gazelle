...
if (check_paranoia_here('seeding+') || check_paranoia_here('leeching+')) {
    ...
    if (check_paranoia('seeding+')) {
        ...
        $CommStats['seeding'] = Text::number_format($Seeding);
    }
    if (check_paranoia('leeching+')) {
        $CommStats['leeching'] = isset($PeerCount['Leeching']) ? Text::number_format($PeerCount['Leeching'][1]) : 0;
    }
}
if (check_paranoia_here('snatched+')) {
    ...
    $CommStats['snatched'] = Text::number_format($Snatched);
    if (check_perms('site_view_torrent_snatchlist', $User['Class'])) {
        $CommStats['usnatched'] = Text::number_format($UniqueSnatched);
    }
...
if (check_perms('site_view_torrent_snatchlist', $Class)) {
    ...
    $CommStats['downloaded'] = Text::number_format($NumDownloads);
    $CommStats['udownloaded'] = Text::number_format($UniqueDownloads);
}
