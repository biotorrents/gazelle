    ...
    <div id="response_<?=$ID?>" class="box">
      <form class="send_form" name="response" id="response_form_<?=$ID?>" action="">
        <div class="head">
          ...
          <input type="text" name="name" id="response_name_<?=$ID?>" size="87" value="<?=Text::esc($Name)?>" />
        </div>
        <div class="pad">
          <div class="box pad hidden" id="response_div_<?=$ID?>">
            <?=Text::parse($Message)?>
          </div>
          <textarea rows="10" cols="87" id="response_message_<?=$ID?>" name="message"><?=Text::esc($Message)?></textarea>
          <br />
