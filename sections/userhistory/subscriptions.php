...
$app->dbOld->prepared_query("
  ...
  FROM `users_subscriptions_comments` AS s
    LEFT JOIN `users_comments_last_read` AS lr ON lr.`UserID` = {$app->userNew->core['id']} AND lr.`Page` = s.`Page` AND lr.`PageID` = s.`PageID`
  ...
  WHERE s.`UserID` = {$app->userNew->core['id']} AND s.`Page` IN ('artist', 'collages', 'requests', 'torrents') AND (s.`Page` != 'collages' OR co.`Deleted` = '0')" . ($ShowUnread ? ' AND c.`ID` > IF(lr.`PostID` IS NULL, 0, lr.`PostID`)' : '') . "
  GROUP BY s.`PageID`)
  UNION ALL
  ...
  FROM `users_subscriptions` AS s
    LEFT JOIN `forums_last_read_topics` AS lr ON lr.`UserID` = {$app->userNew->core['id']} AND s.`TopicID` = lr.`TopicID`
...
?>
      <a href="userhistory.php?action=catchup&amp;auth=<?=$app->userNew->extra['AuthKey']?>"
        class="brackets">Catch up</a>&nbsp;&nbsp;&nbsp;
      <a href="userhistory.php?action=posts&amp;userid=<?=$app->userNew->core['id']?>"
  ...
  <table
    ...
    <tr
      ...
      <td class="avatar" valign="top">
        <?=User::displayAvatar($Result['LastReadAvatar'], $Result['LastReadUsername'])?>
      </td>
      <?php } ?>
