...
class Text
{
    ...
    public static function full_format($Str, $OutputTOC = true, $Min = 3)
    {
        ...
        */
         ...
         */
            ...
            # nl2br() is necessary even though it results in "squashed" BBcode source
            # (it unintelligently converts multiple new lines to multiple line breaks)
    ...
    public static function local_url($Str)
    {
        ...
        // If for some reason your site does not require subdomains
        // or contains a directory in the SITE_DOMAIN,
        // revert to the line below.
        if ($Host === SITE_DOMAIN || $Host === 'www.'.SITE_DOMAIN) {
            if (empty($URLInfo['port']) && preg_match('/(\S+\.)*'.SITE_DOMAIN.'/', $Host)) {
                $URL = '';

                // Things break if the path starts with '//'
                if (!empty($URLInfo['path'])) {
                    $URL .= ltrim($URLInfo['path'], '/');
                }
     ...
     *       However, our quote tag doesn't actually close there.
     *       We must perform a loop which checks the number of opening [quote] tags,
     *       and make sure they are all closed before we find our final [/quote] tag (***).
     *
    ...
    private static function headline_level(&$ItemLevel, &$Level, &$List, $i, &$Offset)
    {
        if ($ItemLevel < $Level) {
            ...
            if ($Offset > 0) {
                $List .= str_repeat('<li><ol>', $Offset - 2);
            }
            ...
            if (self::$Levels < self::$MaximumNests) {
                  ...
                  if (preg_match($Pattern, $Block['Val'], $Matches)) {
                      if (isset($Matches[2])) {
                          ...
                          if ($Groups[$GroupID]) {
                              ...
                              $Name = empty($Group['Name']) ? (empty($Group['Title2']) ? $Group['NameJP'] : $Group['Title2']) : $Group['Name'];
                              $Str .= '>'.$Name.'</a>';
                          } else {
                ...
                case 'img':
                  ...
                  if (!self::valid_url($Block['Val'], '\.(jpe?g|gif|png|bmp|tiff)')) {
                      ...
                      if ($LocalURL) {
                          $Str .= '<img class="lightbox-init" alt="'.$Block['Val'].'" src="'.$LocalURL.'" />';
                      } else {
                          $Str .= '<img class="lightbox-init" alt="'.$Block['Val'].'" src="'.ImageTools::process($Block['Val']).'" />';
                      }
                  }
                  break;
